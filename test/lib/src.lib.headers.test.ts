...
describe('src/lib/headers', () => {
  ...
  describe('parseResponseHeaders', () => {
    ...
    test('should set Content-Type if not set and data is plain object', () => {
      const headers: any = {}
      buildRequestHeaders(headers, { a: 1 })
      expect(headers['Content-Type']).toBe('application/json;charset=utf-8')
    })

    test('should not set Content-Type if not set and data is no plain obejct', () => {
      const headers: any = {}
      buildRequestHeaders(headers, new URLSearchParams('a=b'))
      expect(headers['Content-Type']).toBeUndefined()
    })

    test('should do nothing if headers is undefined or null', () => {
      expect(buildRequestHeaders(undefined, {})).toBeUndefined()
      expect(buildRequestHeaders(null, {})).toBeNull()
    })
  })
