...
describe('lib/url', () => {
  describe('buildRequestURL', () => {
    test('should support null params', () => {
      expect(buildRequestURL('/foo')).toBe('/foo');
    });
    ...
    test('should correct discard url hash mark', () => {
      expect(
        buildRequestURL('/foo?foo=bar#hash', {
          query: 'baz'
        })
    ...
    test('should use serializer if provided', () => {
      ...
      expect(buildRequestURL('/foo', params, serializer)).toBe('/foo?foo=bar');
      expect(serializer).toHaveBeenCalled();
      expect(serializer).toHaveBeenCalledWith(params);
    });

    test('should support URLSearchParams', () => {
      expect(buildRequestURL('/foo', new URLSearchParams('bar=baz'))).toBe('/foo?bar=baz');
    });
  });
